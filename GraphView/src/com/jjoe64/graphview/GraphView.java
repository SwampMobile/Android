...
abstract public class GraphView extends LinearLayout {
	...
	public GraphView(Context context, AttributeSet attrs) {
		this(context, attrs.getAttributeValue(null, "title" ), new GraphViewStyle());
	...
	protected void drawLegend(Canvas canvas, float height, float width) {
		...
		float legendHeight = (shapeSize+15)*graphSeries.size() +15;
		...
		for (int i=0; i<graphSeries.size(); i++) {
			...
			canvas.drawRect(new RectF(lLeft+5, lTop+5+(i*(shapeSize+15)), lLeft+5+shapeSize, lTop+((i+1)*(shapeSize+15))), paint);
			if (graphSeries.get(i).getDescription() != null) {
				paint.setColor(Color.WHITE);
				paint.setTextAlign(Align.LEFT);
				paint.setTextSize(45);
				canvas.drawText(graphSeries.get(i).getDescription(), lLeft+5+shapeSize+15, lTop+shapeSize+10+((i)*(shapeSize+15)), paint);
			}
		}
	}
